...
public class enemyAI : MonoBehaviour
{
    ...
    float velocity; Vector3 playerVelocity;
    Vector3 relativePos;
    Lasers laser;
    ...
    private void shoot()
    {
        ...
        laser = new Lasers(m, 0F, -1 * (100F + velocity), missle, new Vector3(Random.Range(-0.1F, 0.1F), Random.Range(-0.1F, 0.1F), Random.Range(-0.1F, 0.1F)));
    }
    ...
    private void AI()
    {
        ...
        if (Vector3.Distance(transform.position, planetAdjust) < 260F || Vector3.Distance(transform.position, freighter.transform.position) < 20F)// && Vector3.Angle(planet.transform.position, transform.forward) > )
        {
            ...
            transform.rotation = Quaternion.RotateTowards(transform.rotation, rotation, Time.deltaTime * (1 / (Time.deltaTime * 2)));
            ...
            if (attacking)
        {
            ...
            if (timeSinceLastShot > 0.5F)
            {
                ...
                GameObject m = Instantiate(redLaser, missle, mAngle);
                Vector3 laserAngle = new Vector3(transform.rotation.eulerAngles.x, transform.rotation.eulerAngles.y, transform.rotation.eulerAngles.z);
                laser = new Lasers(m, 0F, -1 * (100F + velocity), missle, new Vector3(Random.Range(-0.3F, 0.3F), Random.Range(-0.3F, 0.3F), Random.Range(-0.3F, 0.3F)));
            */
            }
        ...
        else if (avoiding)
        {
            ...
            if (timeAvoiding > 5F)
            {
                ...
                else
                {
                    ...
                    relativePos = (player.transform.position + (playerVelocity * (distance / (100F + velocity)))) - transform.position;
                    Quaternion rotation = Quaternion.LookRotation(relativePos, Vector3.up);
                    transform.rotation = Quaternion.RotateTowards(transform.rotation, rotation, Time.deltaTime * (1 / (Time.deltaTime * 1.9F)));
                }
            }
